...
document.addEventListener('DOMContentLoaded', () => {
	...
	const waitForAFrame = () => {
		if (typeof AFRAME === 'undefined') {
			console.log('A-Frame not loaded yet, waiting...');
			setTimeout(waitForAFrame, 100);
			return;
		}

		console.log('A-Frame loaded, registering wave3-visualizer component');

		// Check if the audio processor exists
		const waitForAudioProcessor = () => {
			const processor = document.getElementById('audio-processor');
			if (
				!processor ||
				!processor.components ||
				!processor.components['audio-processor']
			) {
				console.log('Audio processor not ready yet, waiting...');
				setTimeout(waitForAudioProcessor, 100);
				return;
			}

			console.log(
				'Audio processor ready, registering wave3-visualizer component'
			);
			...
			AFRAME.registerComponent('wave3-visualizer', {
				...
				updateVisualization: function (event) {
					...
					try {
						...
						if (
							this.prevData &&
							this.prevData.length === timeDomainData.length
						) {
							dataToUse = new Uint8Array(timeDomainData.length);
							for (let i = 0; i < timeDomainData.length; i++) {
								dataToUse[i] =
									this.prevData[i] * (1 - this.smoothingFactor) +
						...
						for (let i = 0; i < segments; i++) {
							...
							if (this.data.colorShift) {
								...
								const waveOffset =
									Math.sin((i / segments) * Math.PI * 6 + this.colorPhase * 3) *
										0.5 +
									0.5;
								const intensityFactor =
									this.data.colorShiftIntensity * waveOffset;

								// Modify color based on audio using our own clamp function
								const color = new THREE.Color(positionColor);
								color.r = this.clamp(
									color.r + (bassFactor * 0.3 - 0.15) * intensityFactor,
									0,
									1
								);
								color.g = this.clamp(
									color.g + (trebleFactor * 0.3 - 0.15) * intensityFactor,
									0,
									1
								);
								color.b = this.clamp(
									color.b + (waveOffset * 0.3 - 0.15) * intensityFactor,
									0,
									1
								);
							...
							line.material.opacity =
								0.3 + Math.abs(value) * 0.7 * bassPowerFactor;
						}
					} catch (error) {
				...
				analyzeAudioPower: function (audioData) {
					...
					const newTreblePower = Math.min(
						1.0,
						(trebleSum / (audioData.length - trebleStart)) * 2.0
					);
